...
local Window = Rayfield:CreateWindow({
    Name = "YoxanXHub | Hypershot V2.0.0",
...
mt.__index = function(t, k)
    if getgenv().SilentAimEnabled and tostring(k) == "Hit" then
        ...
        if target and target.Parent and target.Parent:FindFirstChild("Humanoid") then
            ...
            return { Position = target.Position + prediction }
        end
    end
    return oldIndex(t, k)
end
    ...
    if getgenv().BringPlayers then
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                -- Position players slightly offset to avoid collision issues
                plr.Character.HumanoidRootPart.CFrame = CFrame.new(Camera.CFrame.Position + Camera.CFrame.LookVector * 15) * CFrame.new(0, 0, 3)
            end
        end
    end
end)
